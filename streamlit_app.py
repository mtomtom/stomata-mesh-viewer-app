...
def analyze_mesh_detailed(mesh, num_sections=20):
    ...
    p1_cent = parts[1].vertices.mean(axis=0)
    if p0_cent[0] <= p1_cent[0]:
        gc1, gc2 = parts[0], parts[1]
        results['gc_order_swapped_from_file'] = False
    else:
        gc1, gc2 = parts[1], parts[0]
        results['gc_order_swapped_from_file'] = True
    results['gc1'] = gc1
    results['gc2'] = gc2
...
def create_detailed_mesh_plot(results, opacity=0.65, mesh_color="#0072B2", show_wall_vertices=True, show_centreline=True, show_circles=False, show_cross_sections=False, flip_180=False, show_wall_centres=False, show_tip_midsection=False, show_scale_bar=False, scale_bar_length_um=None, unit_label="units", scale_factor=1.0, **kwargs):
    ...
    if show_scale_bar:
        try:
            ...
            # place the label centered over the bar using the bar midpoint
            label_x = (x0 + x1) / 2.0
...
def create_cross_section_png(section_points, section_name, dpi=150, size_inches=(4, 4), title_prefix=None):
    """
    ...
    ax.set_aspect('equal')
    title_text = section_name if title_prefix is None else f"{title_prefix} – {section_name}"
